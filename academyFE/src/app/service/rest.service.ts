...
export class RestService {
  ...
  addDocument(nomeDocumento : string , immagineDocumento : File){

    const formData = new FormData();
    formData.append('immagineDocumento', immagineDocumento);
  
    let params = new HttpParams();
    params = params.append('nomeDocumento', nomeDocumento);

    return this.http.post("http://localhost:8080/api/documento/addDocument",formData, { params })
